            ...
            vertexComboBox.addItemListener(new ItemListener() {
                ...
                public void itemStateChanged(ItemEvent e) {
                    if (e.getStateChange() == ItemEvent.SELECTED) {
                        if (vertexComboBox.getSelectedIndex() != 0) {
                            int selectedIndex = vertexComboBox.getSelectedIndex() - 1; // Adjust index
                            GraphMatrix gmt = new GraphMatrix(countOfVertex);
                            for (Edge item : edgelst) {
                                gmt.addEdges(item);
                            }
                            ...
                            // Displaying results on scrollable labels

                            // Remove any existing result labels
                            for (Component component : getComponents()) {
                                if (component instanceof JLabel && component.getName() != null && component.getName().startsWith("dijkstraResult")) {
                                    remove(component);
                                }
                            }

                            // Create result labels
                            StringBuilder resultsBuilder = new StringBuilder("<html>Dijkstra Shortest Paths:<br/>"); // Use HTML for formatting
                            for (int i = 0; i < countOfVertex; i++) {
                                resultsBuilder.append("v")
                                              .append(i + 1)
                                              .append(": distance = ")
                                              .append(djk.getDistance(i))
                                              .append("<br/>");
                            }
                            resultsBuilder.append("</html>");

                            JLabel resultLabel = new JLabel(resultsBuilder.toString());
                            resultLabel.setName("dijkstraResult"); // Mark the label for easy removal
                            resultLabel.setBounds(buttonX, buttonY + 320, buttonWidth + 100, 200); // Adjust bounds

                            // Add result labels to a scroll pane
                            JScrollPane scrollPaneForResults = new JScrollPane(resultLabel);
                            scrollPaneForResults.setBounds(buttonX, buttonY + 320, buttonWidth + 100, 200); // Adjust bounds

                            add(scrollPaneForResults);
                            revalidate(); // Ensure changes are visible
                            repaint();
                        }
                    }
                }
