                ...
                public void itemStateChanged(ItemEvent e) {
                    if (e.getStateChange() == ItemEvent.SELECTED) {
                        if (vertexComboBox.getSelectedIndex() != 0) {
                            ...
                            List<Integer> distances = djk.getDistanceList();

                            for (int i = 0; i < countOfVertex; i++) {
                                int destinationVertex = i + 1; // Adjust to start from v1

                                if (i != selectedIndex) {
                                    outputMessage.append("Path from v").append(selectedIndex+1)
                                        .append(" to v").append(i+1).append(": ");
                                    for (int node : Dijkstra.paths.get(i)) {
                                        outputMessage.append("v" + (node + 1) + " "); // Adjust vertex numbering
                                    }
                                    outputMessage.append(", Distance " + distances.get(i)).append("\n");
                                }
                            }
